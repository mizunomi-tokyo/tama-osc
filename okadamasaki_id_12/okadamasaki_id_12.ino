...
void loop(){
  if(!connected){
    ...
    if (size > 0) {
      ...
      if (!message.hasError()) {
        message.dispatch("/12/on", led_on);
        message.dispatch("/12/off", led_off);
        message.dispatch("/12/fadein", led_fadein);
        message.dispatch("/12/fadeout", led_fadeout);
        message.dispatch("/12/flash", flash);
