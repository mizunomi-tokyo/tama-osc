...
void loop(){
  if(!connected){
    ...
    if (size > 0) {
      ...
      if (!message.hasError()) {
        message.dispatch("/0/on", led_on);
        message.dispatch("/0/off", led_off);
        message.dispatch("/0/fadein", led_fadein);
        message.dispatch("/0/fadeout", led_fadeout);
        message.dispatch("/0/flash", flash);
