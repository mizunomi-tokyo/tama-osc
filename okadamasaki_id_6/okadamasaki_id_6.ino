...
void loop(){
  if(!connected){
    ...
    if (size > 0) {
      ...
      if (!message.hasError()) {
        message.dispatch("/6/on", led_on);
        message.dispatch("/6/off", led_off);
        message.dispatch("/6/fadein", led_fadein);
        message.dispatch("/6/fadeout", led_fadeout);
        message.dispatch("/6/flash", flash);
