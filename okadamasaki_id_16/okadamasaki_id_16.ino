...
void loop(){
  if(!connected){
    ...
    if (size > 0) {
      ...
      if (!message.hasError()) {
        message.dispatch("/16/on", led_on);
        message.dispatch("/16/off", led_off);
        message.dispatch("/16/fadein", led_fadein);
        message.dispatch("/16/fadeout", led_fadeout);
        message.dispatch("/16/flash", flash);
