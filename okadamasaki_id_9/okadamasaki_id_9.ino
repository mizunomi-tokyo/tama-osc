...
void loop(){
  if(!connected){
    ...
    if (size > 0) {
      ...
      if (!message.hasError()) {
        message.dispatch("/9/on", led_on);
        message.dispatch("/9/off", led_off);
        message.dispatch("/9/fadein", led_fadein);
        message.dispatch("/9/fadeout", led_fadeout);
        message.dispatch("/9/flash", flash);
