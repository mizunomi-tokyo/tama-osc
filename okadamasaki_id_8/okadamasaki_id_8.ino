...
void loop(){
  if(!connected){
    ...
    if (size > 0) {
      ...
      if (!message.hasError()) {
        message.dispatch("/8/on", led_on);
        message.dispatch("/8/off", led_off);
        message.dispatch("/8/fadein", led_fadein);
        message.dispatch("/8/fadeout", led_fadeout);
        message.dispatch("/8/flash", flash);
