...
void loop(){
  if(!connected){
    ...
    if (size > 0) {
      ...
      if (!message.hasError()) {
        message.dispatch("/15/on", led_on);
        message.dispatch("/15/off", led_off);
        message.dispatch("/15/fadein", led_fadein);
        message.dispatch("/15/fadeout", led_fadeout);
        message.dispatch("/15/flash", flash);
